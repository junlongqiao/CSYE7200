...
case class JavaRandomState[T](n: Long, g: Long => T) extends RandomState[T] {
  val r: Random = new Random(n)
  // Hint: Remember to use the "seed" to generate next RandomState.
  // 7 points
  def next: RandomState[T] = JavaRandomState(r.nextLong(),g) // TO BE IMPLEMENTED
  // Hint: Think of the input and output.
  // 5 points
  def get: T = g(n) // TO BE IMPLEMENTED
  // Hint: This one need function composition.
  // 13 points
  def map[U](f: T => U): RandomState[U] = JavaRandomState(n,f compose g) // TO BE IMPLEMENTED
}
...
object RandomState {
  ...
  // 4 points
  val longToDouble: Long => Double = {x => x.toDouble/Long.MaxValue} // TO BE IMPLEMENTED
